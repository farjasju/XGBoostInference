...
int main()
{
    ...
    // Input dataset
    std::ifstream csv_input("1000_samples.csv");
    // Output probabilities file
    std::ofstream output_file;
    output_file.open("fastforest_probs.csv");
    ...
    for (std::string line; getline(csv_input, line);)
    {
        ...
        if (score > 0.5)
            ...
            answer = 0;
        printf("label: %i - score: %i (%f, primary output:%f)\n", label, answer, score, output);
        output_file << filename << ',' << output << ',' << score << '\n';
    }
    output_file.close();
}